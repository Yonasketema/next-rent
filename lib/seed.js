  ...
  try {
    ...
    for (let i = 0; i < 10; i++) {
      users.push({
        ...
        role: faker.helpers.arrayElement([Role.ADMIN, Role.OWNER, Role.USER]),
    ...
    for (let i = 0; i < ethiopianBooks.length - 1; i++) {
      const user = faker.helpers.arrayElement(
        createdUsers.filter((user) => user.role === Role.OWNER),
      );
    ...
    for (let i = 0; i < 41; i++) {
      ...
      const renterUser = faker.helpers.arrayElement(
        createdUsers.filter((user) => user.role === Role.USER),
      );
      rents.push({
        book: {
